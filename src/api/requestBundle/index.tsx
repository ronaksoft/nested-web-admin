...
class RequestBundle {
   ...
   */
  static getInstance(): RequestBundle {
        ...
        setTimeout(() => {
          ...
          this.api.server
            ...
            .then((response: any) => {
              try {
                console.log(
                  `${this.bundleRequestList[domain].request.length} request(s) were bundled`
                );
   ...
   * @memberof RequestBundle
   */
  private getBundleIds(domain: any) {
    const idList: string[] = [];
    const bundleList = this.bundleRequestList[domain];
    bundleList.request.forEach((value: any) => {
   ...
   * @memberof RequestBundle
   */
  private resolveAllPromisesByDomain(domain: any, paramKey: string) {
    const requestList = this.bundleRequestList[domain];
    const responses = requestList.response;
    requestList.request.forEach((request: any) => {
      request.ids.forEach((subValue: any) => {
        const dataList: any[] = [];
   ...
   * @memberof RequestBundle
   */
  private rejectAllPromisesByDomain(domain: any) {
    const requestList = this.bundleRequestList[domain];
    requestList.request.forEach((request: any) => {
