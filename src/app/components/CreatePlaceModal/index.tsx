...
export default class CreatePlaceModal extends React.Component < IProps,
IStates > {
    currentUser : IUser;
    accountApi : any;
    ...
    updatePlaceId(event : any) {
        this.updateModel({id: event.currentTarget.value});
    }

    updatePlaceName(event : any) {
        this.updateModel({name: event.currentTarget.value});
    }
    ...
    pictureChange(info : any) {
        if (info.file.status === 'done') {
            this.updateModel({picture: info.file.response.data[0].universal_id, pictureData: info.file.response.data[0].thumbs});
                    ...
                    <Col className='place-info' span={16}>
                        ...
                        <Row className='input-row'>
                            ...
                            <Input
                                ...
                                .updatePlaceId
                                .bind(this)}/>
                            <p>Place will be identified by this unique address: grand-place.choosen-id You
                                can't change this afterwards, so choose wisely!</p>
                        </Row>
                        <Row className='input-row'>
