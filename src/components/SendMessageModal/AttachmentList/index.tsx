...
import AttachmentApi, { UploadType } from '../../../api/attachment/index';
import Unique from '../../../services/classes/utils/unique';
import Mode from './Item/mode';
// import EMPTY_PICTURE from './default.gif';
import Picture from '../../../services/classes/utils/picture';
...
class AttachmentList extends React.Component<IProps, IState> {
   ...
   */
  public render() {
    const renderItem = (item: IAttachmentItem): JSX.Element => {
    ...
    this.state.items.forEach(item => {
      ...
      if (item.mode === Mode.UPLOAD && item.progress) {
        totalLoaded += item.progress.loaded;
        totalSize += item.progress.total;
        inProgressCount++;
      }

      isUploading = !!item.uploading;
    });
    ...
    return (
      <div className="AttachmentList">
        <div className="AttachmentListTop">
          {totalProgress > 0 && <div className={'totallProgress'}>
            <LinearProgress variant="determinate" value={totalProgress} />
          </div>}
          {isUploading && <span>{getInProgressMessage()}</span>}
          {!isUploading && <span>{getMessage()}</span>}
          <div
            className={['attachListAnchor', this.state.isExpanded ? 'expanded' : null].join(' ')}
            onClick={this.toggleView}
          >
            <IcoN size={24} name="arrow16" />
          </div>
        </div>
        <div>{this.state.isExpanded && items}</div>
      </div>
    );
  }
  ...
  private send(item: IAttachmentItem, file: File, isMedia: boolean) {
    ...
    AttachmentApi.upload(file, type).then((mission: IUploadMission) => {
      ...
      mission.onProgress = (total: number, loaded: number) =>
        this.onUploadProgress(item, {
          loaded,
          total,
        });
